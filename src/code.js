...
(async function () {
  ...
  const submissionsURL =
    "https://kenkoooo.com/atcoder/atcoder-api/results?user=" + userScreenName;
  const submissionsKey = "atcoderDifficultyDisplayUserSubmissions";

  const estimatedDifficulties = await fetchAPIData(
    diffURL,
    diffKey,
    1 * 60 * 60
  );
...
function getElementOfProblemStatus() {
  ...
  for (let i = 0; i < elements_p.length; i++) {
    ...
    if (
      element.textContent.match("メモリ制限:") ||
      element.textContent.match("Memory Limit:")
    ) {
...
function generateDifficultyCircle(rating, isSmall = true) {
  ...
  if (rating < 3200) {
    ...
    // ◒を生成
    return (
      "<span style = 'display: inline-block; border-radius: 50%; border-style: solid;border-width: " +
      borderWidth +
      "px; margin-right: 5px; vertical-align: initial; height: " +
      size +
      "px; width: " +
      size +
      "px;border-color: " +
  ...
  // 金銀銅は例外処理
    ...
    return (
      ...
      "px; width: " +
      size +
      'px; border-color: rgb(150, 92, 44); background: linear-gradient(to right, rgb(150, 92, 44), rgb(255, 218, 189), rgb(150, 92, 44));"></span>'
    );
  } else if (rating < 4000) {
    return (
      '<span style="display: inline-block; border-radius: 50%; border-style: solid;border-width: ' +
      ...
      "px; width: " +
      size +
      'px; border-color: rgb(128, 128, 128); background: linear-gradient(to right, rgb(128, 128, 128), white, rgb(128, 128, 128));"></span>'
    );
  } else {
    return (
      '<span style="display: inline-block; border-radius: 50%; border-style: solid;border-width: ' +
      ...
      "px; width: " +
      size +
      'px; border-color: rgb(255, 215, 0); background: linear-gradient(to right, rgb(255, 215, 0), white, rgb(255, 215, 0));"></span>'
    );
  }
}
...
function changeProblemTitle(
  ...
  if (problem.difficulty != null) {
    ...
    if (problem.is_experimental)
      problemTitle.insertAdjacentHTML("afterbegin", "🧪");
    problemTitle.insertAdjacentHTML(
      "beforebegin",
      generateDifficultyCircle(difficulty, isSmall)
    );
  } else {
...
function addDifficultyText(problemId, estimatedDifficulties, problemStatus) {
  ...
  if (problem.difficulty != null) {
    // 難易度を表示する文字列を生成
    const text = generateDifficultyText(
      problem.difficulty,
      problem.is_experimental
    );
    problemStatus.insertAdjacentHTML("beforeend", " / " + text);
  } else
    problemStatus.insertAdjacentHTML(
      "beforeend",
      " / Difficulty: <span style='font-weight: bold; color: #17a2b8;'>Unavailable</span>"
    );
}
...
function addDifficultyShowButton(placeToAdd, functions) {
  placeToAdd.insertAdjacentHTML(
    "beforebegin",
    `<input id="displayButton" type="button" value="diff"/>`
  );
  document.getElementById("displayButton").onclick = () =>
    displayDifficulty(functions);
}
...
function searchSubmissionsResult(submissions) {
  ...
  for (const item of submissions) {
    const duringContest =
      contestStartTime <= item.epoch_second &&
      item.epoch_second < contestEndTime;
  ...
  return {
    accepted,
    acceptedDuringContest,
    penalties,
    acceptedTime,
    maxPoint,
    maxPointDuringContest,
  };
}
...
function addIsSolvedText(problemId, userSubmissions, problemStatus) {
  const submissions = userSubmissions.filter(function (item, index) {
    if (item.problem_id == problemId) return true;
  });
  ...
  const {
    ...
    maxPoint,
    maxPointDuringContest,
  } = searchSubmissionsResult(submissions);

  let text = "Is Solved: ";
  if (acceptedDuringContest)
    text +=
      "<span style='font-weight: bold; color: white; background: green; border-radius: 20%;'>✓</span>";
  else if (accepted)
    text += "<span style='font-weight: bold; color: green;'>✓</span>";
  else if (submitted)
    text += "<span style='font-weight: bold; color: orange;'>✘</span>";
  else text += "<span style='font-weight: bold; color: gray;'>✘</span>";

  if (acceptedDuringContest)
    text +=
      " <span style='font-size: x-small; color: grey;'>" +
      epochTime2HHMM(acceptedTime - contestStartTime) +
      "</span> ";
